...
//I do a lot of alias setup on the token data, ye be warned. Also sadly there are less emtpy newlines to make this readable due to trying to fit it in discord's limit to see the whole file. I was close to 80 lines over the line limit, soooo.... yeah.
...
for (let key in selectedActors) {
    ...
    //double check whether or not Campaign Notes exists on the token as they don't exist on NPC or Familiar sheets
    try {
    ...
    //If campaign notes aren't empty, and also don't already exist within the dupeCheck Array, then proceed.
    if (campaignNotes != "" && dupeCheck.includes(campaignNotes) != true) {
        ...
        //Modulo calculations on how many can fit left to right, top to bottom.
        x = (i % widthCount) + 1;
        ...
        dialogBoxOptions = {
            top: y * 150 - 100,
            left: x * 200 - 50 }
        //Setup template for the html within the dialog window. Due to using backticks I'll have to explain here instead of each line on what's going on.
        //First few style sets are attempting to find one of the grandparents so we can set various properties including a dark transparent background, minimum width, and auto width.
        //#charImgFor is to format an image of the selected token as well as specifying it is unique to that image if there are multiple dialog windows open. (probably could have been done as a class and I over thought that part)
        //Div section is putting it all together. We also take the image path we found earlier and use it to show it as a small corner image on the dialog.
        //It is also setup so if you were to click on the character image it will open up the character's sheet, because why not?
        ...
        <div class="campaignNotesDialog "><a onClick="game.actors.getName('`+charName+`').sheet.render(true)"><img id="charImgFor`+charName+`"src="`+charImg+`"></a><br>`+ campaignNotes +`</div>`;
        //time to spawn the dialog window
        new Dialog({
            //unique name for each dialog window using the character's name
...
//        Y: `+y+`(`+Math.floor(widthCount / i)+`/`+heightCount+`)`)
        //And at the very end of that large if statement from earlier we just print out a message in console if a sheet's campaign notes already exist in dupeCheck, or is just an empty string.
        } else { console.log("Duplicate or empty notes detected");}}
//Clear out dupeCheck on the event someone decides to have a lot of stuff in their notes.
dupeCheck = [];
